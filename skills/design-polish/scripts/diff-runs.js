...
function diff(beforeDir, afterDir) {
  ...
  const ownHard = (inv, axis) => (inv.tokens.axes[axis].hardcodedOwn != null ? inv.tokens.axes[axis].hardcodedOwn : inv.tokens.axes[axis].hardcoded);
  const hard = (inv) => inv.tokens.colors.values.filter((v) => (v.ownHardcodedCount != null ? v.ownHardcodedCount : v.hardcodedCount) > 0);
  const values = { rawColors: { before: hard(a).length, after: hard(b).length }, rawColorUses: { before: ownHard(a, 'color'), after: ownHard(b, 'color') }, offScaleSpacing: { before: a.tokens.spacing.offScale.length, after: b.tokens.spacing.offScale.length }, rawRadiusUses: { before: ownHard(a, 'radius'), after: ownHard(b, 'radius') }, deadTokens: { before: a.tokens.declared.filter((d) => d.source === 'project' && d.refs.total === 0).length, after: b.tokens.declared.filter((d) => d.source === 'project' && d.refs.total === 0).length }, siblingRadiusMismatches: { before: a.relationships.siblingGroups.filter((g) => g.mismatch.radius && !g.catalog).length, after: b.relationships.siblingGroups.filter((g) => g.mismatch.radius && !g.catalog).length } };
